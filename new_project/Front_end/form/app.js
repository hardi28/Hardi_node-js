...
app.route('/SubmitData')
  .get(function(req, res) {
    fs.readFile("./formData.html", "UTF-8", function(err, html) {
        ...
        .normalizeEmail(), */
            ...
            var errors = validationResult(request);
                if (errors.errors.length>0) {  
                  ...
                  responseHTML += '<button><a href="http://localhost:8000/SubmitData">Back</a></button>';
                  response.send(responseHTML);

                } 
            else{
                console.log(request.body);
                requestData.post({url:'http://localhost:8001/api',
                    method :'POST', 
                    json :(request.body),
                },
