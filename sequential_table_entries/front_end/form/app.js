...
app.route('/SubmitData')
  .get(function(req, res) {
    fs.readFile("./Details.html", "UTF-8", function(err, html) {
        ...
        .normalizeEmail(), */
            ...
            var errors = validationResult(request);
                if (errors.errors.length>0) {  
                  ...
                  responseHTML += '<button><a href="http://localhost:3050/SubmitData">Back</a></button>';
                  response.send(responseHTML);

                } 
            else{
                console.log(request.body);
                requestData.post({url:'http://localhost:3051/api',
                    method :'POST', 
                    json :(request.body),
                },
